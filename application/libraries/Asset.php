...
class asset{
	...
	function stylesheet ( $asset_name = NULL, $media = NULL ) {
		if ( $asset_name != NULL ) {	
			...
			$stylesheet = 	'<link href="' . $asset_url . $asset_name . '.css" rel="stylesheet" type="text/css"';
	
			if ($media != NULL) {
				$stylesheet = $stylesheet . ' media="' . $media . '" />';
			}
			else {
	...
	function javascript ( $asset_name = NULL ) {
		if ( $asset_name != NULL ) {
			...
			$javascript = 	'<script type="text/javascript" src="' . $asset_url . $asset_name . '.js"></script>';					
			echo $javascript;
		}
	}
	...
	function image ( $asset_name = NULL, $alt = NULL, $additional_elements = NULL ) {
		if ( $asset_name != NULL) {
			...
			$image = '<img src="' . $asset_url . $asset_name . '"';
			if ( $alt != NULL ) {
				$image = $image . ' alt="' . $alt . '"';
			}
